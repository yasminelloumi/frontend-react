...
function ListePatient() {
  ...
  const getPatientHistorique = async (id) => {
    ...
    setError("");
    try {
      const response = await axios.get(`http://localhost:5026/api/Patient/${id}`);
    ...
    <Container className="mt-5">
      ...
      <Modal show={showModal} onHide={closeModal} centered>
        <Modal.Header closeButton>
          <Modal.Title>Historique Médical de {selectedPatient?.namePatient}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
